# Описание пакета Primo.Office.OdfOxml
...
Все перечисленные ниже элементы добавляются в рабочую область внутри элемента **Документ ODF**. Это элемент, производящий подключение к приложению ODF Документы. Элемент **Документ ODF** включает в себя следующие свойства:
...
Элемент, вставляющий таблицу в документ. Работает корректно только внутри контейнера. Включает в себя следующие свойства:
...
Элемент, производящий запись данных в документ Word. Работает корректно только внутри контейнера. Включает в себя следующие свойства:
...
### Элемент "Копировать в буфер обмена"
...
#### Элемент "Переименовать страницу"
...
### Элемент "Записать формулу в ячейку"
...
1. **Таблица**:
   ...
   - Формат — определяет формат, в котором следует скопировать данные. Доступны следующие значения:
      ...
      - Formulas — будут скопированы значения и формулы. Чтобы значение формул корректно отображалось при вставке, после копирования диапазона необходимо добавить элемент **Пересчет формул**.
      - Formats — будет скопирован только формат ячеек, шрифт и цвета. Значения и формулы проигнорируются.

### Элемент "Найти начальную/конечную строку"
...
### Элемент "Найти ячейку"

Элемент ищет ячейку с заданным текстом  в диапазоне ячеек страницы. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
### Элемент "Обновить сводные таблицы"

Производит обновление сводных таблиц. Работает корректно только в контейнере **Таблица ODF**.

### Элемент "Пересчитать формулы"

Пересчитывает формулы таблиц. Работает корректно только в контейнере **Таблица ODF**.

### Элемент "Редактировать диаграмму"

Элемент редактирует диаграмму в таблице. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
Элемент сохраняет файл таблиц. Если путь к файлу не указан, сохранен будет файл текущего контейнера **Таблица ODF**. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:

1. **Таблица**:
   - Путь к файлу — по кнопке ![alt text](image-3.png) можно указать путь к файлу в окне Проводника.

### Элемент "Сохранить как PDF"
...
### Элемент "Фильтр диапазона"

Элемент устанавливает фильтр диапазона ячеек таблицы. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
### Элемент "Формула из ячейки"

Элемент читает формулу из ячейки. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
Элемент считывает данные из диапазона ячеек ODF-таблицы. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
### Элемент "Чтение из ячейки"

Элемент читает данные из ячейки страницы. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
...
### Элемент "Чтение колонки"

Элемент считывает данные из колонки ODF-таблицы. Работает корректно только в контейнере **Таблица ODF**. Включает в себя следующие свойства:
